/*
  ...
  1. New Columns
    ...
    - Add match_score to matches table
    - Add message_type to messages table
    - Add difficulty_level and estimated_hours to study_plans table
    - Add recording_url to sessions table
    - Add tags to reviews table

  2. New Tables
    - user_preferences: Store user settings and preferences
    - session_files: Store files associated with sessions
    - call_sessions: Track video/audio call sessions
...
-- Insert enhanced sample subjects (check if they exist first)
DO $$
BEGIN
  -- Insert React Development if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'React Development') THEN
    INSERT INTO subjects (name, category) VALUES ('React Development', 'STEM');
  END IF;
  
  -- Insert Machine Learning if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'Machine Learning') THEN
    INSERT INTO subjects (name, category) VALUES ('Machine Learning', 'STEM');
  END IF;
  
  -- Insert Creative Writing if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'Creative Writing') THEN
    INSERT INTO subjects (name, category) VALUES ('Creative Writing', 'Humanities');
  END IF;
  
  -- Insert Business Administration if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'Business Administration') THEN
    INSERT INTO subjects (name, category) VALUES ('Business Administration', 'Business');
  END IF;
  
  -- Insert Marketing if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'Marketing') THEN
    INSERT INTO subjects (name, category) VALUES ('Marketing', 'Business');
  END IF;
  
  -- Insert Finance if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM subjects WHERE name = 'Finance') THEN
    INSERT INTO subjects (name, category) VALUES ('Finance', 'Business');
  END IF;
END $$;
...
DO $$
BEGIN
  -- Insert First Connection if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'First Connection') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('First Connection', 'Made your first tutor match', 'social', 100, '{"matches": 1}');
  END IF;
  
  -- Insert Study Streak if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'Study Streak') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('Study Streak', 'Completed 7 consecutive days of study sessions', 'study', 200, '{"consecutive_days": 7}');
  END IF;
  
  -- Insert Knowledge Seeker if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'Knowledge Seeker') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('Knowledge Seeker', 'Completed 10 study sessions', 'progress', 150, '{"sessions_completed": 10}');
  END IF;
  
  -- Insert Top Performer if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'Top Performer') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('Top Performer', 'Achieved 90%+ average in assessments', 'achievement', 300, '{"average_score": 90}');
  END IF;
  
  -- Insert Helpful Tutor if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'Helpful Tutor') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('Helpful Tutor', 'Received 5-star rating from 10 students', 'social', 250, '{"five_star_ratings": 10}');
  END IF;
  
  -- Insert Subject Master if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM achievements WHERE name = 'Subject Master') THEN
    INSERT INTO achievements (name, description, category, points, requirements) 
    VALUES ('Subject Master', 'Completed advanced level in any subject', 'achievement', 400, '{"advanced_completion": 1}');
  END IF;
END $$;
...
RETURNS trigger AS $$
BEGIN
  -- Insert profile (will be ignored if it already exists due to unique constraint)
  BEGIN
  ...
  -- Create default user preferences (will be ignored if they already exist due to unique constraint)
  BEGIN
    INSERT INTO public.user_preferences (user_id)
    VALUES (new.id);
  EXCEPTION WHEN unique_violation THEN
    -- Preferences already exist, do nothing
  END;
